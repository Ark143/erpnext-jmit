...
def application(request: Request):
	...
	try:
		...
		if request.method == "OPTIONS":
			...
			try:
				url = f"http://127.0.0.1:9000{request.full_path}"
				req = urllib.request.Request(url, data=request.get_data() if request.method in ("POST", "PUT") else None, headers={k: v for k, v in request.headers if k.lower() not in ("host", "content-length")}, method=request.method)
				with urllib.request.urlopen(req, timeout=5) as resp:
					response = Response(resp.read(), status=resp.status, content_type=resp.headers.get("Content-Type", "text/plain"))
			except Exception:
...
def application_with_statics():
	...
	sites_path = _sites_path or getattr(frappe.local, "sites_path", None) or "sites"
	if os.path.exists(os.path.abspath(os.path.join(sites_path, "sites", "assets"))):
		assets_path = os.path.abspath(os.path.join(sites_path, "sites", "assets"))
	else:
		assets_path = os.path.abspath(os.path.join(sites_path, "assets"))

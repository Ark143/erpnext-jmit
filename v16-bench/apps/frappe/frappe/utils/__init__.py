...
def get_assets_json():
	def _get_assets():
		...
		import os
		possible_paths = [
			os.path.abspath(os.path.join(frappe.get_site_path(), "..", "sites", "assets", "assets.json")),
			os.path.abspath("sites/assets/assets.json"),
			frappe.get_site_path("..", "sites", "assets", "assets.json"),
		]
		content = None
		...
		if not content:
			content = frappe.read_file("assets/assets.json")

		assets = frappe.parse_json(content) or {}

		rtl_path = os.path.abspath("sites/assets/assets-rtl.json")
		if os.path.exists(rtl_path):
			try:
				with open(rtl_path, "r") as f:
					if rtl_data := frappe.parse_json(f.read()):
						if isinstance(rtl_data, dict):
							assets.update(rtl_data)
			except Exception:
				pass
	...
	if not frappe.conf.developer_mode:
		cached_assets = frappe.client_cache.get_value(
			"assets_json",
			shared=True,
